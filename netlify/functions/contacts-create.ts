...
export default async (req: Request) => {
  try {
    ...
    if (!first_name || !phone_e164) {
      return badRequest("Missing first_name or phone_e164");
    }
    if (!/^\+[\d]{6,16}$/.test(phone_e164)) {
      return badRequest("phone_e164 must be in E.164 format (es. +39...)");
    }

    // upsert su vincolo unico phone_e164: se esiste aggiorna i nomi
