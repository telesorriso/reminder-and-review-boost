...
// token per funzioni Netlify (se lo usi)
function authHeaders(): HeadersInit {
  // prova 1: localStorage
  const fromLS = typeof window !== "undefined" ? window.localStorage.getItem("admin_token") : null
  // prova 2: input visibile nella UI (id="admin-token")
  const fromInput =
    typeof document !== "undefined"
      ? (document.getElementById("admin-token") as HTMLInputElement | null)?.value
      : null

  const token = (fromLS || fromInput || "").trim()
  return token ? { "x-api-key": token } : {}
}

const displayName = (a: RawAppointment) => {
  // priorità: contatto (Cognome Nome) > patient_name > telefono > "Sconosciuto"
  const full = [a.contact?.last_name, a.contact?.first_name].filter(Boolean).join(" ").trim()
  return full || (a.patient_name?.trim() || "") || a.contact?.phone_e164 || a.phone_e164 || "Sconosciuto"
}
...
function layoutWithLanes(items: UiAppointment[]): Positioned[] {
  ...
  for (const appt of sorted) {
    ...
    // filtra gli scaduti mentre posizioniamo
    active = active.filter(a => a.endMin > startMin)
    active.push({ appt: { ...appt, topMin: Math.max(0, startMin - DAY_START_MIN), heightMin, lane, laneCount: 1 }, endMin })
  }
  flush()
  return out
}
  ...
  return (
    <div style={{ position: "relative", width: 64 }}>
      {labels.map((t, i) => (
        <div
          key={t}
          style={{ position: "absolute", top: i * SLOT_MIN, transform: "translateY(-50%)", fontSize: 12, color: "#666" }}
        >
          {t}
        </div>
      ))}
...
function CreateModal({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        const j = await r.json()
        if (!cancel) setResults((Array.isArray(j.items) ? j.items : j.contacts) || [])
      } catch {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSaved() // -> ricarica e fa comparire la CARD
      onClose()
    } catch (e: any) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 8 }}>
          <label>
            <input type="radio" checked={mode === "contact"} onChange={() => setMode("contact")} /> Usa contatto
          </label>
          <label>
            <input type="radio" checked={mode === "manual"} onChange={() => setMode("manual")} /> Inserisci manualmente
          </label>
        </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {!!results.length && !selected && (
                <div style={{ border: "1px solid #eee", borderRadius: 6, marginTop: 6, maxHeight: 200, overflow: "auto" }}>
                  {results.map(c => {
                    const name = [c.last_name, c.first_name].filter(Boolean).join(" ").trim() || "(senza nome)"
                    return (
                      <button
                        type="button"
                        key={c.id}
                ...
                >
                  <div>
                    <div style={{ fontWeight: 600 }}>
                      {[selected.last_name, selected.first_name].filter(Boolean).join(" ").trim() || "(senza nome)"}
                    </div>
